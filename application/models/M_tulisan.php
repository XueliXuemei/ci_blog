...
class M_tulisan extends CI_Model
{
	...
	{
		$hsl = $this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_tulisan ORDER BY tanggal DESC LIMIT 1");
		return $hsl;
	}
	function berita_all()
	{
		$hsl = $this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%M. %d, %Y') AS tanggal, tbl_pengguna.pengguna_nama AS penulis FROM tbl_tulisan,tbl_pengguna WHERE tbl_tulisan.tulisan_pengguna_id =tbl_pengguna.pengguna_id ORDER BY tanggal DESC");
		return $hsl;
	}
	...
	{
		$hsl = $this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%M. %d, %Y') AS tanggal FROM tbl_tulisan where tulisan_id='$kode'");
		return $hsl;
	}
