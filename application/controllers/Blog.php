...
class Blog extends CI_Controller
{
	...
	function index()
	{
		...
		$kode = 1;
		$x['berita'] = $this->m_tulisan->berita_all();
		...
		$this->template->load('template', 'depan/v_blog', $x);
	}
	function detail($slugs)
	{
		...
		if ($query->num_rows() > 0) {
			...
			$x['category'] = $this->db->query("SELECT tbl_kategori.kategori_id, tbl_kategori.kategori_nama AS nama, COUNT(tbl_tulisan.tulisan_kategori_id) AS jum FROM tbl_kategori LEFT  JOIN tbl_tulisan ON tbl_kategori.kategori_id = tbl_tulisan.tulisan_kategori_id");
			$x['populer'] = $this->db->query("SELECT * FROM tbl_tulisan ORDER BY tulisan_views DESC LIMIT 5");
			$x['menu'] = $this->m_halamanstatis->get_menu_halamanstatis();
			$x['identias'] = $this->m_identitas->get_identitas($kode = 1);
			$this->template->load('template', 'depan/v_blog_detail', $x);
		} else {
			redirect('artikel');
		}
	}
	...
	function kategori()
	{
		...
		$query = $this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%M. %d, %Y') AS tanggal,  tbl_pengguna.pengguna_nama AS penulis FROM tbl_tulisan, tbl_pengguna WHERE tbl_tulisan.tulisan_pengguna_id=tbl_pengguna.pengguna_id AND tulisan_kategori_nama LIKE '%$kategori%' ORDER BY tulisan_views DESC LIMIT 5");
		if ($query->num_rows() > 0) {
			$x['berita'] = $query;
			$x['identias'] = $this->m_identitas->get_identitas($kode = 1);
			$x['category'] = $this->db->query("SELECT tbl_kategori.kategori_id, tbl_kategori.kategori_nama AS nama, COUNT(tbl_tulisan.tulisan_kategori_id) AS jum FROM tbl_kategori LEFT  JOIN tbl_tulisan ON tbl_kategori.kategori_id = tbl_tulisan.tulisan_kategori_id");
